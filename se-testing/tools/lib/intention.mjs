...
// 保证「意图 schema」是单一来源，不在两个脚本里各写一份。
...
export function loadIntentions(projectDir) {
  const dir = path.join(projectDir, INTENTIONS_DIR);
...
export function loadSpecs(projectDir) {
  const dir = path.join(projectDir, SPECS_DIR);
...
export function resolveFixturePlaceholder(projectDir, expr) {
  ...
  const fixturePath = path.join(projectDir, FIXTURES_DIR, `${fileKey}.json`);
...
export function fixturePath(projectDir, fixtureFile) {
  return path.join(projectDir, FIXTURES_DIR, fixtureFile);
}
