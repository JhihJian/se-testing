...
| Agent 流程分为意图生成、journey 分支索引、spec 实现、执行失败分析和审计报告。 | `se-testing/skills/testing-intent/SKILL.md`、`se-testing/skills/testing-journey/SKILL.md`、`se-testing/skills/testing-spec/SKILL.md`、`se-testing/skills/testing-run/SKILL.md` | 已确认 |
...
| 意图文件必须使用 YAML，目标项目的意图目录为 `intentions/`，也支持 `tests/intentions/` 作为资产根。 | `se-testing/tools/lib/intention.mjs`、`se-testing/tools/validate-intention.mjs` | 已确认 |
...
| `check-intentions` 负责严格意图字段、kebab-case 标识和空泛描述检查。 | `se-testing/tools/check-intentions.mjs`、`se-testing/tools/check-intentions.test.mjs` | 已确认 |
| `check-journeys` 负责 journey schema、intent 引用、spec 不直接绑定 journey 和质量 warning。 | `se-testing/tools/check-journeys.mjs`、`se-testing/tools/check-journeys.test.mjs` | 已确认 |
...
| 四个校验脚本输出 JSON，有错误时以非零退出码结束；warning 不阻断但必须被报告或 critic 回应。 | `se-testing/tools/validate-intention.mjs`、`se-testing/tools/check-intentions.mjs`、`se-testing/tools/check-journeys.mjs`、`se-testing/tools/check-binding.mjs` | 已确认 |
...
| `report.md` 必须包含意图链、四个校验脚本输出或 artifact 索引、diff 摘要和 Playwright 证据。 | `se-testing/template/report.md`、`se-testing/skills/testing-run/SKILL.md` | 已确认 |
| 插件内 `examples/login-fixture` 和根目录 `e2e-fixture-project` 是登录验证夹具。 | `se-testing/examples/login-fixture/`、`e2e-fixture-project/` | 已确认 |
| `run-dogfood.mjs` 会运行示例 Playwright 测试、四个校验脚本并重写示例报告。 | `se-testing/tools/run-dogfood.mjs` | 已确认 |
