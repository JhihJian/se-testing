...
`se-testing` 是一个面向 Codex Agent 的测试工程能力包。它向 Agent 提供测试意图生成、Playwright spec 编写、测试执行、失败分析和审计报告生成的流程能力，同时提供可独立运行的 Node 校验脚本和可复制到业务项目的测试骨架。
...
- 审计报告：`report.md`，收敛 Playwright 证据、两个校验脚本输出、diff 摘要和失败处置记录。
...
1. Agent 读取需求，按 `testing-intent` 生成 draft 意图，并用 `validate-intention` 做 schema、fixture 和占位符自检。
2. critic 在意图定稿前做对抗审查，人确认业务语义后，意图从 `draft` 进入 `reviewed`。
3. test-author 按 `testing-spec` 将 reviewed/active 意图实现为 Playwright spec，写入版本绑定头和 assertion 标记。
4. test-runner 按 `testing-run` 运行 Playwright，执行层失败可自主修 Page Object 或 fixture；疑似意图层问题只能升级 critic 并产出提案。
5. 收工前必须运行 `validate-intention` 和 `check-binding`，并把 stdout 原样写入 `report.md`，再由 critic 复核报告和证据。
...
- `validate-intention` 与 `check-binding` 输出结构化 JSON，作为报告中的机器证据。
