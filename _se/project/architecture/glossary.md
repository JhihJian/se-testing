...
| journey | `intentions/journeys/*.yaml`，描述同一业务路径下的分支、公共前置条件、已覆盖 intent 和已知缺口。 | `se-testing/skills/testing-journey/SKILL.md` |
...
| check-intentions | 严格校验意图审计字段、kebab-case 标识和空泛描述的 Node 脚本。 | `se-testing/tools/check-intentions.mjs` |
| check-journeys | 校验 journey schema、intent 引用、spec 不直接绑定 journey 并输出 warning 的 Node 脚本。 | `se-testing/tools/check-journeys.mjs` |
...
| report.md | runner 收工前产出的可审计报告，包含 Playwright 结果和四个校验脚本输出或 artifact 索引。 | `se-testing/template/report.md` |
